/* Very simplistic gateway.  Doesn't recover well if killed, for example. */
#include "hex.h"
#include "json.h"
			 ...
			 const char *opt3)
{
	...
	case 0:
		...
		execlp(name, name, arg1, req, opt1, opt2, opt3, NULL);
		exit(errno);
	}
...
static char *ask_bitcoind(const tal_t *ctx,
			  ...
			  const char *opt3)
{
	return ask_process(ctx, "bitcoin", "-testnet", req, opt1, opt2, opt3);
}
					...
					const jsmntok_t *txid)
{
	...
	*/
	txstr = tal_fmt(ctx, "%.*s", 
			json_tok_len(txid), json_tok_contents(buffer, txid));
	...
	/* This can happen in if someone aims their block reward at
	   the gateway. */
	intxid = json_get_member(buffer, toks, ".vin[0].txid");
	onum = json_get_member(buffer, toks, ".vin[0].vout");
	if (!intxid || !onum)
	...
	txstr = tal_fmt(ctx, "%.*s", 
			json_tok_len(intxid),
			json_tok_contents(buffer, intxid));
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		char *fromstr = tal_fmt(this_ctx, "from=%u", known_txs);

		toks = json_bitcoind(this_ctx, &buffer,
				     "listtransactions", "gateway",
				     "count=999999", fromstr);
		...
		*/
		...
		end = json_skip_elem(&toks[0]);
		for (t = toks + 1; t < end; t = json_skip_elem(t+1)) {
			...
			if (!pettycoin_tx(ctx, privkey, destaddr,
				...
				errx(1, "Pettcoin injection failed for"
