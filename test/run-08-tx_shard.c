...
void check_chains(struct state *state) { fprintf(stderr, "check_chains called!\n"); abort(); }
...
void merkle_some_txs(const struct block_shard *shard,
		     ...
		     struct protocol_double_sha *merkle) { fprintf(stderr, "merkle_some_txs called!\n"); abort(); }
...
void update_block_ptrs_new_block(struct state *state, struct block *block) { fprintf(stderr, "update_block_ptrs_new_block called!\n"); abort(); }
